...
@Component({
  ...
  animations: [
    trigger('listaResultados', [
      ...
      transition(':leave', [
        animate('130ms ease-in', style({ opacity: 0, transform: 'translateY(-4px) scale(0.99)' }))
      ])
    ]),
    trigger('itemAnim', [
      transition(':enter', [
        style({ opacity: 0, transform: 'translateY(10px)' }),
        animate('170ms {{delay}}ms cubic-bezier(0.2, 0, 0, 1)', style({ opacity: 1, transform: 'translateY(0)' }))
      ], { params: { delay: 0 } })
    ])
  ]
})
export class BuscarCancionesComponent implements OnInit, OnChanges {
  ...
  configurarBuscador() {
    this.buscador$.pipe(
      ...
      switchMap(texto => {
        if (!texto || texto.length < 3) { this.buscando = false; return of([]); }
        return this.spotifyService.buscarTracks(texto);
      })
    ).subscribe({
      next: (canciones: any) => { this.tracks = canciones; this.buscando = false; },
      error: (err) => { console.error(err); this.buscando = false; }
    });
  }
  ...
  onSearchChange() {
    ...
    if (busqueda.length === 0) { this.tracks = []; this.buscando = false; this.buscador$.next(''); return; }
    if (busqueda.length >= 3) { this.buscando = true; this.buscador$.next(busqueda); }
    else { this.tracks = []; this.buscando = false; }
  }
